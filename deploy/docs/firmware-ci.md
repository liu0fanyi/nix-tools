...
配置已按用户允许完成：仅main分支环境回查通过后才保存秘密；受限接收器与公钥已安装并通过真实只读/负向检查。GitHub套餐不支持指定审批人门禁，当前采用用户确认的仅main限制，无需再次询问该配置授权。实际CI发行结果见009任务。

正式首发已通过：0.1.14，运行35513938725；CI默认公网下载成功，独立回查使用正常域名TLS1.2，原5条记录完整保留。重复上传使用同一份已签候选，不能重建不同字节覆盖相同版本。后续新版本由产品更新版本后手动运行release工作流，默认publish=false；固件最低手机兼容版本必须核对。
