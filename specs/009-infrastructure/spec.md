...
统一固件增量：接受`esp32_device_bean`作为独立产品，旧三产品仅保留已有目录兼容；统一版来源兼容由产品候选工具保证。新增统一版本不得移除旧记录或扩大写入路径。正式CI权限仍独立于本机候选发布，未配置不得宣称Actions自动发布完成。

正式CI上传须使用独立SSH公钥的强制命令，仅接受单条统一固件事务。旧三产品只保留已有目录，禁止借此写Android/Clip、删版本、写任意路径或运行shell。发布处理器属于基础设施运行程序，不是产品源码；服务器仅保存该处理器、上传公钥和公开制品，绝不保存签名/上传私钥。
